...
- Get text input via the _standard input_ and output HTML
  via the _standard output_
- Receive two file names as _command-line arguments_, read the contents
...
Originally, Haskell was designed as an _open standard_
...
In GHC Haskell, we use a _lazy evaluation strategy_ to implement non-strict
semantics (We've talked about laziness
[before](../04-markup/02-parsing-01.md#laziness)).
